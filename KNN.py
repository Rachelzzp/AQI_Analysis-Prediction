...
class KNearestNeighbor(object):
    ...
    def Run(self):
        ...
        testSet = []
        split = 0.75
        self.loadDataset('data.txt', split, trainingSet, testSet)  # 数据划分
        ...
        for x in range(len(testSet)):  # 对所有的测试集进行测试
            neighbors = self.getNeighbors(trainingSet, testSet[x], k)  # 找到4个最近的邻居
            result = self.getResponse(neighbors)  # 找这4个邻居归类到哪一类
